...
def initialize(robot_xml = None, env_path = None, viewer = 'rviz',sim = True, **kw_args):
    ...
    if env_path is not None:
        ...
        env.RemoveKinBody(env.GetKinBody('fetch'))
    ...
    urdf_uri = 'package://fetchpy/robot_description/fetch.gazebo2.urdf'
    ...
    #Default arguments
    keys = ['arm_sim','arm_torso_sim','gripper_sim','head_sim','torso_sim','base_sim','talker_sim','perception_sim','whole_body_sim']

    for key in keys:
        if key not in kw_args:
            kw_args[key] = sim

    prpy.bind_subclass(robot, FETCHRobot, robot_checker_factory = robot_checker_factory, **kw_args)

    if sim:
        dof_indices, dof_values = robot.configurations.get_configuration('arm_dock')
        robot.SetDOFValues(dof_values, dof_indices)
        robot.gripper.SetDOFValues([0.05,0.05])
